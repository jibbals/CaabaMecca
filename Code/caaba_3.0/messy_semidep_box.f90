! Time-stamp: <2011-02-14 16:43:48 sander>
...
MODULE messy_semidep_box
  ...
  USE caaba_mem,                 ONLY: model_time, zmbl, timesteplen, c
...
CONTAINS
  ...
  SUBROUTINE emission
    ...
    ! JESSE added emissions scenarios
    CASE ('AUS_FOR')
      CALL emission_aus_for
    CASE DEFAULT
  ...
  CONTAINS
    ...
    !-------------------------------------------------------------------------
    ! JESSE change: adding emissions scenario 
    ! numbers are in emissions molecules/cm2/second
    ! May need to convert GEOS-Chem atomsC/cm2/second to molecules
    SUBROUTINE emission_aus_for
      IF (ind_O3     /= 0) c(ind_O3)     = c(ind_O3)     + 5.E10 * fct ! ref0203, p. 6699
      IF (ind_NO     /= 0) c(ind_NO)     = c(ind_NO)     + 5.E10 * fct
      IF (ind_C5H8   /= 0) c(ind_C5H8)   = c(ind_C5H8)   + 1.E11 * fct
    END SUBROUTINE emission_aus_for
    
    ! can make a holding scenario which sets NOx to constants
    !                                       1ppb * air molecs/cm3 * fct
    ! fct=change in mixing ratio per time step conversion
    ! IF (ind_NO     /= 0) c(ind_NO)     = 1.0E-9 * 2.5E19 * fct
    ...
    SUBROUTINE emission_mim2
      !IF (ind_O3     /= 0) c(ind_O3)     = c(ind_O3)     + 5.E10 * fct ! ref0203, p. 6699
      IF (ind_NO     /= 0) c(ind_NO)     = c(ind_NO)     + 0.333E10 * fct
  ...
  SUBROUTINE drydep
    ...
    CASE ('')
      CALL drydep_default
    ! JESSE added depositions scenarios
    CASE ('AUS_FOR')
      CALL drydep_aus_for
    CASE DEFAULT
  ...
  CONTAINS
    ...
    !-------------------------------------------------------------------------
    ! JESSE : dry dep 1. - fct* dep velocity in cm/s
    SUBROUTINE drydep_aus_for
    ...
    SUBROUTINE drydep_mim2
      ! value for O3 is consistent with South America in Fig. 8, Kerkweg
      ! et al., ACP 6, 4617, 2006:
